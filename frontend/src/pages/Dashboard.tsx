...
export const Dashboard: React.FC = () => {
  const { assets, allocations, transfers, maintenance, addAsset, allocateAsset } = useAppState();
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
        ...
        <KPICard title="Allocated" value={countAllocated} icon={TrendingUp} iconBgColor="bg-[#0d6efd]/10" iconColor="text-[#0d6efd]" />
        <KPICard title="Available" value={countAvailable} icon={CheckCircle2} iconBgColor="bg-[#198754]/10" iconColor="text-[#198754]" />
        <KPICard title="Pending Transfers" value={countPendingTransfers} icon={RefreshCw} iconBgColor="bg-[#ffc107]/10" iconColor="text-[#b25e00]" />
        <KPICard title="Active Repairs" value={countRepairs} icon={Wrench} iconBgColor="bg-[#dc3545]/10" iconColor="text-[#dc3545]" />
        <KPICard title="Capital Value" value={`$${totalValue.toLocaleString()}`} icon={DollarSign} iconBgColor="bg-[#198754]/10" iconColor="text-[#198754]" />
      </div>
            ...
            {recentActivities.map(act => (
              <div key={act.id} className="flex items-start gap-3 border-b border-[#dee2e6] pb-3 last:border-0 last:pb-0 select-none">
                ...
                <div className="min-w-0">
                  <h4 className="text-[14px] font-bold text-[#212529]">{act.title}</h4>
                  <p className="text-xs text-[#6c757d] mt-0.5 leading-relaxed">{act.desc}</p>
        ...
        <Card title="Overdue Asset Allocations">
          <div className="space-y-4">
            {activeAllocations.length === 0 ? (
              <p className="text-xs text-[#6c757d]">No active allocations detected.</p>
            ) : (
                ...
                {activeAllocations.map(alc => (
                  <div key={alc.id} className="flex justify-between items-center border-b border-[#dee2e6] pb-3 last:border-0 last:pb-0">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[#6c757d] font-semibold mt-1">Allocated to {alc.allocatedTo} ({alc.department})</p>
                    </div>
        ...
        <Card title="Active Maintenance Requests" headerActions={
          ...
          <div className="space-y-4">
            {criticalRepairs.length === 0 ? (
              <p className="text-xs text-[#6c757d]">No active repair tickets.</p>
            ) : (
              <div className="space-y-3.5 select-none">
                {criticalRepairs.map(rep => (
                  <div key={rep.id} className="flex justify-between items-center border-b border-[#dee2e6] pb-3 last:border-0 last:pb-0">
                    <div>
                      ...
                      <p className="text-xs text-[#6c757d] font-semibold mt-1">Priority: <span className="font-extrabold text-[#dc3545]">{rep.priority}</span></p>
                    </div>
      ...
      <Modal isOpen={isRegOpen} onClose={() => setIsRegOpen(false)} title="Register New Asset">
        <form onSubmit={regForm.handleSubmit(onRegisterSubmit)} className="space-y-4">
          <Input label="Asset Name" placeholder="e.g. ThinkPad X1" error={regForm.formState.errors.name?.message} {...regForm.register('name')} />
          <Input label="Serial Number" placeholder="e.g. S12345" error={regForm.formState.errors.serialNumber?.message} {...regForm.register('serialNumber')} />
          ...
          <Input label="Purchase Value ($)" type="number" placeholder="2000" error={regForm.formState.errors.purchaseValue?.message} {...regForm.register('purchaseValue')} />
